...
hold on 
trisurf(in,y+6,x-6,zeros(size(x)),v,'facecolor','interp','linestyle',':'); % Plot flat surface colored by v
% ANother option:
...
if(nargin>4)
    ...
    if(size(Vec,1)==ne) % Element centroid vectors
        ...
        flipped_centx = centy + 6;
        flipped_centy = centx - 6;
        quiver3(flipped_centx,flipped_centy,zeros(ne,1)+range(centx)*0.0001,-Vec(:,2),-Vec(:,1),zeros(ne,1),Vecscale,'k-'); % Plot vectors slightly above surface
    end
end
