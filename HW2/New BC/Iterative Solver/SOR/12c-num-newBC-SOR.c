...
Problem 12c - with Type III Boundary Condition - with JACOBI Point Iteration
      ...
      for (int j=0; j<N; j++){
        for(int i=0; i<N; i++){
          ...
          if(row == 0){    //Type II corner
            ...
            bij = B[row];
            //printf("Corner\n");
            U1[row] = (omega/beta0) * (beta1 * U0[rPlus] + beta3 * U0[thetaPlus] - bij + (1.0-omega)*U0[row]);    //Jacobi
          }
            ...
            bij = B[row];
            //printf("Boundary I\n");
            U1[row] = (omega/beta0) * (beta1 * U0[rPlus] + beta2 * U1[rMinus] + beta4 * U1[thetaMinus] - bij + (1.0-omega)*U0[row]);
          }
            ...
            bij = B[row];
            //printf("Boundary III\n");
            U1[row] = (omega/beta0) * (beta1 * U0[rPlus] + beta2 * U1[rMinus] + beta3 * U0[thetaPlus] - bij + (1.0-omega)*U0[row]);
          }
            ...
            bij = B[row];
            //printf("Boundary II\n");
            U1[row] = (omega/beta0) * (beta1 * U0[rPlus] + beta3 * U0[thetaPlus] + beta4 * U1[thetaMinus] - bij + (1.0-omega)*U0[row]);
          }
          else{
            beta0 = - a;
            beta1 = b;
            ...
            bij = B[row];
            //printf("Interior\n");
            U1[row] = (omega/beta0) * (beta1 * U0[rPlus] + beta2 * U1[rMinus] + beta3 * U0[thetaPlus] + beta4 * U1[thetaMinus] - bij + (1.0-omega)*U0[row]);
          }
